...
describe("resolveConfigDir", () => {
  it("respects RATEBOARD_CONFIG_DIR override", () => {
    const prev = process.env["RATEBOARD_CONFIG_DIR"];
    process.env["RATEBOARD_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["RATEBOARD_CONFIG_DIR"];
      else process.env["RATEBOARD_CONFIG_DIR"] = prev;
    }
  });
});
