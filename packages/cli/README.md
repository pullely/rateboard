...
`rateboard` — first-class TypeScript CLI for the Rateboard control
...
This package is internal to the `rateboard` monorepo and is not
...
```
rateboard login    [--api-url=URL] [--token=BEARER]
rateboard logout
rateboard whoami
```
...
```
rateboard org list
rateboard org use <org-id>
rateboard org members
rateboard project list
```

Writes (Task 0101):

```
rateboard org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
rateboard project create <name> [--idempotency-key=KEY]
rateboard env create <project-id> <name> [--idempotency-key=KEY]
rateboard api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
rateboard webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
rateboard usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
rateboard billing summary
rateboard audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`rateboard org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `rateboard login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/rateboard/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/rateboard/config.json` (mode 0644, not a secret). Override
both via `RATEBOARD_CONFIG_DIR` (used by tests).
