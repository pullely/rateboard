// Base HTTP transport for the Rateboard SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `RateboardError`
...
export interface ClientOptions {
  /**
   * Base URL of the Rateboard api-edge worker, e.g.
   * `https://api.rateboard.app`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Rateboard` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Rateboard SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Rateboard SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
