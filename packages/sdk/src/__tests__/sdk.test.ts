...
import { Rateboard } from "../index.js";
import {
  BadRequestError,
  ConflictError,
  ForbiddenError,
...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Rateboard({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => RateboardError]> = [
  ...
  it("forward-compat: unknown error code → base RateboardError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(RateboardError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as RateboardError).code).toBe("future_quota_exceeded");
    }
  });
