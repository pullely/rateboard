...
Rateboard TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Rateboard control plane API.

## Install (workspace)

This package is internal to the `rateboard` monorepo. Add it as a
...
```ts
import { Rateboard } from "@saas/sdk";

const client = new Rateboard({
  baseUrl: "https://api.rateboard.app",
  auth: { kind: "bearer", token: process.env["RATEBOARD_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `RateboardError`. Branch on
...
| `RateboardError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `RateboardError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Rateboard({ fetch })`.
