...
Your backend holds one Rateboard API key and **zero GitHub credentials**.
...
const rateboard = new Rateboard({
  baseUrl: process.env.RATEBOARD_API_URL!,
  token: process.env.RATEBOARD_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await rateboard.integrations.issueGithubToken(ORG_ID, {
