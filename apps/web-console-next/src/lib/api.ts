...
// deleted; all wire I/O now goes through the typed `Rateboard` resource
...
//   - `createClient` — constructs a `Rateboard` against a target, wired
...
//     `Promise<T>` + thrown `RateboardError` into the same shape the
...
import { Rateboard, RateboardError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Rateboard;
...
export function createClient(target: ApiTarget, token: string | null): Rateboard {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Rateboard(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `RateboardError` subclasses surface as `{ ok: false }` carrying the
...
export async function wrap<T>(fn: () => Promise<T>): Promise<ApiResult<T>> {
  try {
    ...
    if (e instanceof RateboardError) {
